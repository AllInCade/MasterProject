...
full_model_lin <- glmmTMB(Male_death~ Age +Year,disp=~Age,offset = log(Male_Exp),family = nbinom2(), 
...
full_model_glmmTMB <- glmmTMB(Male_death~ s(Age) +s(Year),disp=~Age,offset = log(Male_Exp),family = nbinom2(), 
                              data = full_train_data)
...
print(paste("RMSE for lin:", RMSEGAMM))
