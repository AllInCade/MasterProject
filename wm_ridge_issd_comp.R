...
wind_data <- wind_data %>%
  ...
  filter(datetime_UTC >= as.POSIXct("2022-01-01") & datetime_UTC <= as.POSIXct("2022-04-01")) %>%
...
total_rows <- nrow(wind_data_clean)
train_size <- floor(train_proportion * total_rows)
valid_size <- floor(valid_proportion * total_rows)

train_data <- wind_data_clean[1:train_size, ]
valid_data <- wind_data_clean[(train_size + 1):(train_size + valid_size), ]
test_data <- wind_data_clean[(train_size + valid_size + 1):total_rows, ]
...
fit_and_evaluate_on_subset <- function(model_formula, model_family, full_train_data, valid_data, proportion, augmented = FALSE, augmented_train_data = NULL, use_REML = FALSE) {
  ...
  fit_and_evaluate_model(model_formula, train_subset, valid_data, model_family, use_REML)
}
...
learning_curve_data <- function(data, valid_data, proportions, augmented_data = NULL) {
  future_map(proportions, function(prop) {
    list(
      ...
      AutoModel = fit_and_evaluate_on_subset(auto_model_formula, auto_model_family, data, valid_data, prop, use_REML = FALSE)
    )
  })
}
