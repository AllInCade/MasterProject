...
covariates<-c("Income", "TotIncome", "Age", "Income_log", "TotIncome_log" ,"Face","Education", "Charity")

insurance_data_clean <- na.omit(insurance_data)
write_csv(insurance_data_clean, "wind_data_cleaned.csv")
rm(insurance_data)
gc()
sample_interval <- 1  # Can be adjusted as needed
merged_df <- insurance_data_clean[seq(1, nrow(insurance_data_clean), sample_interval), ]

rf_model <- randomForest(Face ~ Income_log +TotIncome_log+Education+Gender, data = merged_df, ntree = 1000, importance = TRUE)
...
augment_data <- function(data, lambda, n_knots_list) {
  ...
  y_augmented <- augment_response(data$Face, max(sapply(augmented_matrices, nrow)))
  
  augmented_data <- data.frame(Face = y_augmented)  # Ensure correct response variable name
  for (name in names(n_knots_list)) {
...
gcv_lambda <- function(lambda, n_knots_list, data) {
  ...
  gcv_model <- glmmTMB(formula = Face ~ 1 + Xr_Income_log+Xr_TotIncome_log, 
                       data = augmented_data, family = gaussian())
...
manual_model_formula <- formula(Face ~ 1 + Xr_Income_log+Xr_TotIncome_log,
                                data = augmented_data)
manual_model_family <- gaussian(link="identity")

auto_model_formula <- formula(Face ~ s(Income_log)+s(TotIncome_log), 
                              data = augmented_data)
...
fit_and_evaluate_model <- function(model_formula, train_data, valid_data, family, use_REML = FALSE) {
  ...
  metrics <- list(
    RMSE = calculate_metrics(valid_data$Face, predictions),
    TimeTaken = timing["elapsed"]
  )
  return(metrics)
}
...
cat(sprintf("Final Manual Model - RMSE: %f, Training Time (s): %f, Prediction Time (s): %f\n", 
            calculate_metrics(test_data$Face, final_manual_model$Predictions), 
...
cat(sprintf("Final Auto Model - RMSE: %f, Training Time (s): %f, Prediction Time (s): %f\n", 
            calculate_metrics(test_data$Face, final_auto_model$Predictions), 
...
plot_predictions <- function(model_predictions, test_data, models_to_train) {
  prediction_df <- data.frame(Actual = test_data$Face)
  rmse_values <- list()
  timing_values <- list()
