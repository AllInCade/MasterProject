...
hist(motorins$Claims, breaks = 3000)
...
model <- randomForest(Claims~Insured+Make+Bonus+Kilometres, data = train_data, na.action = na.omit, importance = TRUE)

predictions <- predict(model, test_data)
actuals <- test_data$Claims
...
glmmTMB_model<-glmmTMB(Claims~s(Insured)+Bonus+(1|Zone),
                       data=motorins,tweedie())
...
gamm4_model<- gamm4(Claims~s(Insured)+Bonus,random=~(1|Zone),data=motorins,family = Tweedie(p=xi))
...
hist(test_data$Claims, breaks=1000)
hist(train_data$Claims, breaks=1000)
hist(valid_data$Claims, breaks=1000)

# Training tmb_nb on train_data
tmb_nb_train <- glmmTMB(Claims~s(Insured)+Bonus+ (1|Zone),disp=~Kilometres,family = tweedie(),
                        data = train_data)


# Training gamm_nb on train_data
gamm_nb_train <- gamm4(Claims~s(Insured)+Bonus,random=~(1|Zone),family = Tweedie(p=xi), 
                       data = train_data) 
...
for(i in 1:n_folds) {
  ...
  # Train GAMM model
  model_gamm <- gamm4(Claims~s(Insured)+Bonus,random=~(1|Zone),family = Tweedie(p=xi), 
                      data = train_fold)
  pred_gamm <- predict(model_gamm$gam, test_fold, type = "response")
  rmse_gamm[[i]] <- sqrt(mean((pred_gamm - test_fold$Claims)^2))
  
  # Train GLMMTMB model
  model_glmmTMB <- glmmTMB(Claims~s(Insured)+Bonus+ (1|Zone),disp=~Kilometres,family = tweedie(),
                           data = train_fold)
  pred_glmmTMB <- predict(model_glmmTMB, test_fold, type = "response")
  rmse_glmmTMB[[i]] <- sqrt(mean((pred_glmmTMB - test_fold$Claims)^2))
}
...
full_model_gamm <- gamm4(Claims~s(Insured)+Bonus,random=~(1|Zone),family = Tweedie(p=xi), data = full_train_data)
...
full_model_glmmTMB <- glmmTMB(Claims~s(Insured)+Bonus+ (1|Zone),disp=~Kilometres,family = tweedie(), 
...
actual_density <- density(full_train_data$Claims)
...
# Your existing ggplot code with added coord_cartesian for zooming in
ggplot() +
  ...
  facet_wrap(~Model)+
  coord_cartesian(xlim = c(0,700), ylim = c(0,0.2)) # Adjust as needed
