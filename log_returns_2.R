...
for (col in grep("N100", names(merged_df), value = TRUE)) {
  if (!grepl("lag", col) && !grepl("log_return", col)) {  # Exclude log return already lagged
    ...
    # After lagging, you may want to handle NA introduced at the start of each lagged column
    # For instance, you can decide to fill these with the first non-NA value backward or forward or leave as NA
    # This depends on how critical it is for your model not to have NAs
