...
getSymbols("EQNR", from = "2014-01-01")

EQNR_df <- as.data.frame(`EQNR`)
EQNR_df <- na.aggregate(EQNR_df)
colnames(EQNR_df) <- c("open", "high", "low", "close", "volume", "adjusted")
EQNR_df$time <- 1:nrow(EQNR_df)
EQNR_df$date <- index(`EQNR`)
EQNR_df$year <- format(EQNR_df$date, "%Y")
EQNR_df$month <- format(EQNR_df$date, "%m")
EQNR_df$log_return <- c(NA, diff(log(EQNR_df$close)))

fit_data <- na.omit(EQNR_df)
